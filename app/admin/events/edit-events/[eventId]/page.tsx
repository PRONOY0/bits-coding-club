...
const EditParticular = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      const eventId = localStorage.getItem("id") as string
      if (!eventId) return;
      ...
      try {
        ...
        if (response.status === 200) {
          reset({
            ...
            date: response.data.event.date ? new Date(response.data.event.date) : new Date(),
            time: response.data.event.time || '',
            image: response.data.event.image || null,
          });
          setValue("category", response.data.event.category || '');

          if (response.data.event.image) {
            setImagePreview(response.data.event.image);
          }
